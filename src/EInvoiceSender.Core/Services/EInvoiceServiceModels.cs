...
{
    /// <summary>Die Eingangs-PDF wird geprüft.</summary>
    Preflight,

    /// <summary>Die erfassten Rechnungsdaten werden geprüft.</summary>
    ValidateInvoiceData,

    /// <summary>Die Rechnungs-XML wird erzeugt.</summary>
    CreateXml,

    /// <summary>Die erzeugte XML wird gegengeprüft.</summary>
    VerifyXml,

    /// <summary>Die PDF/A-3-Datei mit eingebetteter XML wird erzeugt.</summary>
    ComposePdfA,

    /// <summary>Die fertige Datei wird erneut geöffnet und ausgelesen.</summary>
    ReopenAndExtract,

    /// <summary>Externe Validatoren prüfen die fertige Datei.</summary>
    ExternalValidation,
...
{
    /// <summary>Der Schritt läuft gerade.</summary>
    Running,
    ...
    /// <summary>Der Schritt wurde übersprungen.</summary>
    Skipped,
}

/// <summary>
/// Fortschrittsmeldung für die Oberfläche.
/// </summary>
...
/// </summary>
/// <param name="SourcePdfPath">Die vorhandene PDF-Rechnung. Wird nie verändert.</param>
/// <param name="Invoice">Die erfassten und bestätigten Rechnungsdaten.</param>
...
/// Gewünschter Dateiname. Leer bedeutet: aus Rechnungsnummer und Empfänger
/// bilden.
/// </param>
...
/// Bestätigung, dass eine bereits in der PDF enthaltene Rechnungs-XML ersetzt
...
/// <param name="WasExecuted">Wurde es tatsächlich ausgeführt?</param>
/// <param name="Note">Erläuterung, falls es nicht ausgeführt wurde.</param>
...
/// </summary>
/// <param name="Succeeded">Wurde eine gültige Datei erzeugt und gespeichert?</param>
...
/// <param name="CompletedSteps">Zustand je Schritt, für die Anzeige.</param>
...
/// <param name="Validators">Beteiligte Prüfwerkzeuge mit Version.</param>
...
///
/// Die Oberfläche kennt nur diese eine Schnittstelle. Alles Weitere –
/// Eingangsprüfung, Regelwerk, XML-Erzeugung, PDF/A-Aufwertung, Berichte –
/// liegt dahinter und wird von <c>EInvoiceService</c> zusammengeführt.
///
/// Die drei Methoden entsprechen den drei Fragen, die die Oberfläche stellt:
/// Kann ich diese PDF verwenden? Sind die erfassten Daten in Ordnung? Und:
/// bitte erzeuge die E-Rechnung.
/// </summary>
public interface IEInvoiceService
{
    /// <summary>Prüft, ob eine PDF-Datei als Grundlage taugt.</summary>
    Task<PdfPreflightReport> AnalyzePdfAsync(
        string pdfPath,
        CancellationToken cancellationToken = default);

    /// <summary>
    /// Prüft die erfassten Rechnungsdaten gegen das Regelwerk EN 16931.
    ///
    /// Läuft ohne Datei- und Prozesszugriff und ist deshalb während der
    /// Eingabe jederzeit aufrufbar.
    /// </summary>
    ValidationReport ValidateInvoice(Invoice invoice);

    /// <summary>Erzeugt und prüft die E-Rechnung.</summary>
