...
/// <summary>
/// Erzeugt ein gültiges, minimales ICC-v2-Farbprofil für sRGB.
///
...
/// Herkunft und Lizenz gesondert dokumentiert werden müsste, wird das Profil
...
///
/// Aufbau nach ICC.1:2001-04 (ICC-Version 2.4), Geräteklasse "Monitor",
/// Matrix/TRC-Modell:
/// Kopf (128 Byte), Tag-Tabelle, danach die Tag-Daten. Enthalten sind die für
/// ein RGB-Matrix-Profil vorgeschriebenen Tags: desc, wtpt, cprt, rXYZ, gXYZ,
/// bXYZ, rTRC, gTRC, bTRC.
///
/// Die Primärvalenzen sind die bekannten, auf den Weißpunkt D50 adaptierten
...
public static class SRgbIccProfile
{
    ...
    // Weißpunkt D50, wie er im ICC-Profilverbindungsraum vorgeschrieben ist.
    ...
    /// die übliche Näherung für die sRGB-Übertragungsfunktion in einem
    /// einfachen Kurventag.
    /// </summary>
    ...
    /// identisch, damit zwei Läufe dieselbe Ausgabedatei erzeugen.
    /// </summary>
    ...
    private static byte[] Build()
    {
        ...
        // erzeugte Datei zwischen zwei Läufen byte-identisch bleibt.
    ...
    private static void WriteHeader(byte[] profile, int totalSize)
    {
        ...
        // 12: Geräteklasse "Monitor"
        WriteSignature(header[12..], "mntr");

        // 16: Datenfarbraum RGB (mit abschließendem Leerzeichen)
        WriteSignature(header[16..], "RGB ");

        // 20: Profilverbindungsraum XYZ
        WriteSignature(header[20..], "XYZ ");

        // 24: Erzeugungszeitpunkt. Fest gewählt, damit das Profil und damit
        // auch die Prüfsumme der Ausgabedatei reproduzierbar bleiben.
        WriteDateTime(header[24..], new DateTime(2026, 1, 1, 0, 0, 0, DateTimeKind.Utc));

        // 36: Dateikennung
        WriteSignature(header[36..], "acsp");

        // 40: Primärplattform – keine bestimmte
        // 44: Profilkennzeichen – nicht eingebettet, unabhängig verwendbar
        // 48: Gerätehersteller, 52: Gerätemodell, 56: Geräteeigenschaften
        ...
        // 80: Erzeuger, 84: Profil-Prüfsumme (zulässigerweise null),
    ...
    private static byte[] BuildTextDescription(string text)
    {
        ...
        int asciiLength = ascii.Length + 1; // einschließlich Nullterminierung

        // Aufbau: Signatur(4) + reserviert(4) + ASCII-Länge(4) + ASCII-Text
        //         + Unicode-Sprachcode(4) + Unicode-Länge(4)
        //         + ScriptCode-Code(2) + ScriptCode-Länge(1) + ScriptCode-Text(67)
    ...
    /// <summary>Wandelt eine Fließkommazahl in die ICC-Festkommadarstellung s15Fixed16.</summary>
